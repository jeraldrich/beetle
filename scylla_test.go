...
func TestScylla(t *testing.T) {
	...
	createMessagesTable(t, session)
}

func createTestKeyspace(t *testing.T, session gocqlx.Session) {
	err := session.ExecStmt(`CREATE KEYSPACE IF NOT EXISTS test_db WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}`)
	if err != nil {
...
func createMessagesTable(t *testing.T, session gocqlx.Session) {
